 ...
 * 服务器:
 * ip:162.14.74.178
 * user: guest, passwd: helloworld(无权限)
 * user: lihua, passwd: lizehua21(有权限)
...
//======================================================================
	...
	//连接服务器
	if (connect(hSock, (SOCKADDR*)&servAdr, sizeof(servAdr)) == SOCKET_ERROR)// ! 卡在这里连不上了 
	{
		printf("connect error : %d", GetLastError());
		return -1;
	}else {
		printf("欢迎你，我的朋友。请输入你的聊天用户名：");
		//加上连接成功提示音
	}
